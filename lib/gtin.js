...
// Databases store the same barcode in different lengths (USDA: mostly 12
// digits, some 13, 14, 11 with the leading zero dropped, or 8). Only ever
// add or drop LEADING ZEROS, so a different product can never match.
export function barcodeForms(gtin14, code, typed) {
  const forms = [gtin14, code, typed, gtin14.replace(/^0+/, '')];
  if (gtin14.startsWith('0')) forms.push(gtin14.slice(1));
  if (gtin14.startsWith('00')) forms.push(gtin14.slice(2));
  return [...new Set(forms.filter(Boolean))];
}

// True when two barcode strings differ only by leading zeros.
export const sameBarcode = (a, b) => String(a).replace(/^0+/, '') === String(b).replace(/^0+/, '');
...
export function normalizeBarcode(raw, format = '') {
  ...
  // Some USDA records hold a US UPC-A WITHOUT its check digit (11 digits).
  // Matched by exact string only, since the check digit is fully determined.
  const exactForms = kind === 'UPC-A' ? [code.slice(0, -1)] : [];
  return { ok: true, code, gtin14, ean13, kind, storeLabel, forms: barcodeForms(gtin14, code, digits), exactForms };
}
