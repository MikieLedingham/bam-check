...
import { normalizeBarcode, sameBarcode } from './gtin.js';
...
// gtins: one barcode string or the list of forms it may be stored under
// (differing only by leading zeros). exact: forms that must match character
// for character (the check-digit-less UPC).
export function pickUsdaMatch(json, gtins, exact = []) {
  const forms = [].concat(gtins);
  const foods = (json?.foods || []).filter((f) => {
    const stored = String(f.gtinUpc || '');
    return stored !== '' && (forms.some((g) => sameBarcode(stored, g)) || exact.includes(stored));
  });
...
async function fetchUsda(bc, key, signal) {
  const q = new URLSearchParams({
    ...
    query: [...bc.forms, ...bc.exactForms].join(' '), // every way USDA might have stored this barcode
    dataType: 'Branded',
    pageSize: '10',
  });
  const { json, notFound } = await getJson(`https://api.nal.usda.gov/fdc/v1/foods/search?${q}`, signal);
  if (notFound) return null;
  return normalizeUsda(pickUsdaMatch(json, bc.forms, bc.exactForms));
}
